...
class Config:
    ...
    def __post_init__(self):
        # Parse clinic numbers if provided, else default to filter keys
        ...
        # Define operatory filters keyed by clinic
...
class AppointmentData:
    ...
    def from_api_response(cls, data: Dict[str, Any]) -> 'AppointmentData':
        ...
        dtstr = data.get('AptDateTime', '')
        stamp = data.get('DateTStamp', '')
        pattern = data.get('Pattern', '')
        apt_dt = parse_dt(dtstr) or datetime.datetime.utcnow()
        date_t = parse_dt(stamp) or datetime.datetime.utcnow()
        # Duration: count X's * 10min default
        dur = max(15, min(pattern.count('X')*10 if pattern.count('X') else 60, 480))
        end_dt = apt_dt + datetime.timedelta(minutes=dur)
        return cls(
            apt_num=aptnum,
            pat_num=patnum,
            clinic_num=clinic,
...
def fetch_patient_details(pat:int)->Dict[str,Any]:
    if not pat: return {}
    try:
        r = requests.get(f"{config.api_base_url}/patients/{pat}", headers=make_headers(), timeout=config.request_timeout); r.raise_for_status(); return r.json()
    except:
        r = requests.get(f"{config.api_base_url}/patients", headers=make_headers(), params={'PatNum':pat}, timeout=config.request_timeout); r.raise_for_status()
        d=r.json(); return d[0] if isinstance(d,list) and d else (d if isinstance(d,dict) else {})
...
def send_to_keragon(a: AppointmentData, pd: Dict[str, Any]) -> bool:
    ...
    # remove any keys whose value is empty/None/0
...
# === CORE SYNC ===
def run_sync()->bool:
    if not config.developer_key or not config.customer_key or not config.keragon_webhook_url or not config.clinic_nums:
        logger.critical("Missing configuration vars"); return False
    ...
    for clinic in config.clinic_nums:
        ...
        # build range
        start = now.strftime("%Y-%m-%d"); end = (now + datetime.timedelta(days=config.first_run_lookahead_days)).strftime("%Y-%m-%d")
        appts=[]
        for stat in ['Scheduled','Complete','Broken']:
            for op in config.clinic_operatory_filters.get(clinic, [None]):
                try:
                    appts += fetch_appointments_batch(clinic,start,end,stat,op)
                except Exception as e:
                    logger.error(e)
        objs=[AppointmentData.from_api_response(x) for x in appts]
        to_send = objs if first_run else [x for x in objs if x.date_t_stamp>=since]
        # fetch patients once
        cache={};
        for a in to_send:
            cache.setdefault(a.pat_num, fetch_patient_details(a.pat_num))
        for a in to_send:
...
def main():
    ...
    if args.reset:
        try: os.remove(config.state_file); logger.info("State reset")
        except: logger.info("No state to reset")
        return
    if args.dump_state:
        print(open(config.state_file).read() if os.path.exists(config.state_file) else "No state file")
        return
    if args.test:
        run_sync(); return
